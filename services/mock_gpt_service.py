...
class MockGPTService:
    ...
    def generate_agent_response(
        ...
        if phase == 'calc':
            body = f"선택하신 목표({goal})에 맞춰 영양·에너지 가이드를 만들었어요. 아래 기준으로 잡았습니다."
            return finish(body, "이 가이드대로 식단 구성을 진행할게요.",
        ...
        if phase == 'sleep':
            lead = "수면 개선이 핵심 목표라 더 꼼꼼히 설계했어요. " if goal == '수면의 질 개선' else ""
            body = lead + "수면 리듬과 생활습관 루틴을 정리했어요."
            return finish(body, "이 루틴대로 2주 일정을 편성할게요.",
