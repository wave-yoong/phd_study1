...
class MockGPTService:
    ...
    def generate_agent_response(
        ...
        if phase == 'delivery':
            ...
            if has_diet:
                lines.append("- 식단: 하루 약 1,900kcal, 균형 잡힌 구성")
            lines.append("- 운동: 주 5일(근력+유산소), 수/일 휴식")
            lines.append("- 수면·생활습관: 취침 23:30 / 기상 07:00, 스트레칭·산책 루틴")
            if has_diet:
                lines.append("- 1주차 장보기 리스트 포함")
            lines.append("\n일자별 루틴은 위 일정 편성 결과를 따르고, 1·8·14일차에 컨디션을 점검하세요.\n")
            lines.append("안전 안내: 무리한 절식은 피하고, 어지럼증 등 이상이 있으면 강도를 낮추세요. "
                         "지속 가능한 습관이 가장 중요합니다.")
            return "\n".join(lines)
