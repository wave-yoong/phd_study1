...
class GPTService:
    ...
    **출처 표기 규칙 (매우 중요)**:
    ...
    링크 제공 규칙:
    - 정확한 원문 페이지 URL만 제공 (예: 특정 기사, 논문, 보고서의 직접 링크)
    - 홈페이지나 섹션 페이지 링크는 절대 금지 (예: https://www.mfds.go.kr 같은 홈페이지)
    - 정확한 URL을 모르면 링크 없이 출처명만 표기
    - 애매한 링크를 주는 것보다 링크 없이 출처만 명시하는 게 훨씬 낫습니다
    
    출처 표기 형식:
    1. 정확한 URL을 아는 경우:
       "[출처: 식품의약품안전처 - 올리브유 품질기준 고시 제2023-45호, 2023.06.15, https://www.mfds.go.kr/brd/m_99/view.do?seq=12345]"
    
    2. 정확한 URL을 모르는 경우 (이게 더 정직합니다):
       "[출처: 대한영양학회 - 건강 지방 섭취 가이드라인, 2024 (해당 기관 웹사이트에서 검색 가능)]"
       또는
       "[출처: 연합뉴스 2024년 1월 보도 (정확한 링크 불확실)]"
    
    답변 구성:
    1. 앞서 제안한 구조에 맞춰 완전한 답변 제공
    2. 각 섹션마다 출처를 명확히 표기
    3. 답변 마지막에 "참고 자료" 섹션 추가 (정확한 링크가 있는 것만)
    
    4. 마지막: "추가로 궁금하신 점이 있으신가요?"

    예시:
    좋은 예: "올리브유는 심혈관 건강에 도움이 됩니다 [출처: American Heart Association - Dietary Fats and Cardiovascular Disease, 2017, https://www.ahajournals.org/doi/10.1161/CIR.0000000000000510]"
    
    나쁜 예: "올리브유는 심혈관 건강에 도움이 됩니다 [출처: 대한의학회, https://www.kamje.or.kr]" ← 홈페이지 링크는 의미없음
    ...
    중요:
    - 부정확한 링크는 사용자를 혼란스럽게 합니다
    - 링크 없이 출처명만 주는 것이 잘못된 링크보다 낫습니다
    - 전문적이면서도 친근하게"""
        }
