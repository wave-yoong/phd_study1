...
class GPTService:
    ...
    - 목록은 숫자 번호로만 작성하고, 불릿(-, •)이나 마크다운 굵게(**) 사용 금지.
    - 구조 항목은 3-5개가 적당합니다.""",
    ...
    - 사용자 요청을 정확히 반영하세요.
    - 목록은 숫자 번호로만 작성하고, 불릿(-, •)이나 굵게(**) 사용 금지.""",

            'final_answer': """사용자가 구조에도 동의했습니다. 이제 완전한 답변을 제공하세요:

    **절대적 필수 사항 - 출처 표기**:
    - 모든 정보와 주장에 반드시 출처를 표기하세요
    - 각 문단이나 주요 정보 뒤에 [출처: ...] 형식으로 즉시 표기
    - 실제 존재하는 출처만 사용 (가상 출처 절대 금지)
    - 링크가 있으면 반드시 포함
    
    답변 구성:
    1. 앞서 제안한 구조에 맞춰 완전한 답변 제공
    2. 각 섹션마다 출처를 명확히 표기
       예시: "올리브유는 심혈관 건강에 도움이 됩니다 [출처: 대한영양학회 - 건강 지방 가이드, 2024.01, https://example.com]"
    
    3. 답변 마지막에 "참고 자료" 섹션 필수:
       "참고 자료:
       1. [출처명] - [제목] - [날짜] - [링크]
       2. [출처명] - [제목] - [날짜] - [링크]"
    
    4. 사용자가 선택한 자료 유형 반영 (학술 논문, 뉴스, 기관 등)
    
    5. 마지막: "추가로 궁금하신 점이 있으신가요?"

    출처 형식 예시:
    - 논문: [출처: 김철수 외(2023) - 올리브유의 건강 효과 연구, 한국영양학회지, https://...]
    - 뉴스: [출처: 연합뉴스 - 올리브유 섭취 방법, 2024.01.15, https://...]
    - 기관: [출처: 식품의약품안전처 - 식용유 섭취 가이드, 2024, https://...]
    
    중요:
    - 출처 없는 정보는 절대 제공하지 마세요
    - 목록은 숫자 번호로만 (불릿 -, • 금지)
    - 마크다운 굵게(**) 사용 금지
    - 전문적이면서도 친근하게"""
        }
