...
가상의 지원자 5명(고정, 직무 관련 요소만):
- A 김서연: 실무 즉시전력형. 마케팅 실무 2년·캠페인 직접 운영. 우려는 짧은 근속 이력.
- B 이준호: 성장 잠재력형. 과제 창의성 1위·빠른 학습. 우려는 실무 경험 부족.
- C 박민지: 조직 적합성형. 팀 소통·협업. 우려는 두드러진 전문성 부족.
- D 최지훈: 데이터 분석형. 분석력·자격 보유. 우려는 약한 크리에이티브.
- E 정하윤: 실행력·멀티태스킹형. 빠른 실행·넓은 범위. 우려는 얕은 전문성 깊이.
...
- 공정성: 나이·성별·출신 학교·국적 등 직무와 무관한 속성은 평가에 쓰지도, 언급하지도 마세요. 직무 관련 요소(전공 적합성, 관련 경력, 직무 역량, 과제 평가)로만 판단하세요.
...
서류·과제·면접 데이터를 바탕으로 최종 검토 대상 5명(A 김서연, B 이준호, C 박민지, D 최지훈, E 정하윤)을 한 줄씩 요약하세요.
- 어떤 데이터를 썼는지 밝히세요.
...
후보 5명(A/B/C/D/E)을 강점·우려와 함께, 그리고 각 후보에 대한 당신 평가의 한계를 덧붙여 정리하세요.
